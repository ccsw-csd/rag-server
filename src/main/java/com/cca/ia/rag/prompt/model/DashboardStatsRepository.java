...
public interface DashboardStatsRepository extends Repository<PromptEntity, Long> {
    ...
    @Query(value = "select min(id) as id, tag as label, count(1) as value from prompt_tag group by tag order by count(1) desc limit 10", nativeQuery = true)
    List<DashboardStatsDto.ChartData> getTopTags();

    @Query(value = "select min(ap.id) as id, concat(ap.name, ' ', ap.lastname) as label, count(1) as value from prompt p join personal.all_person ap on p.person_id = ap.id group by concat(ap.name, ' ', ap.lastname) order by count(1) desc limit 10", nativeQuery = true)
